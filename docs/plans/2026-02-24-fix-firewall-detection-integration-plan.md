...
- [ ] After creating `event_detector`, call `event_detector.set_firewall_coordinator(firewall_coordinator.clone())` if firewall detection is enabled
- [ ] Store the polling request sender (not drop it — see Task 2) and call `event_detector.set_polling_request_sender(sender.clone())`
- [ ] Verify `event_detector.start_monitoring()` is called during broker startup
...
- [ ] Remove the `_` prefix — store the sender as `polling_request_sender`
- [ ] Pass it to `event_detector.set_polling_request_sender()` (from Task 1)
- [ ] Store it in the `EventBroker` struct if other components need to send polling requests
- [ ] Verify `start_polling_request_processing(polling_request_receiver)` correctly processes incoming requests
...
- [ ] When event timeout is detected for a registration:
...
- [ ] Handle the case where the speaker/service pair is not found (log warning, skip)
- [ ] Add a `registration_pairs: HashMap<RegistrationId, SpeakerServicePair>` or pass the registry reference to EventDetector
- [ ] Add method `register_pair(registration_id, pair)` called when a new subscription is created
- [ ] Add method `unregister_pair(registration_id)` called when a subscription is removed
- [ ] Add test verifying that event timeout sends a polling request
...
- [ ] Remove `firewall_detection_timeout` field (never read anywhere)
- [ ] Remove `firewall_detection_retries` field (never read anywhere)
- [ ] Remove `firewall_detection_fallback` field (never read anywhere)
- [ ] Update all config presets that set these values
