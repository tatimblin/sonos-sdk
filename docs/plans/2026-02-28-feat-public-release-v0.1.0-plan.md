...
- [x] **Run dry-run** — `chmod +x scripts/publish.sh && ./scripts/publish.sh --dry-run`
- [x] **Verify** — all 9 crates pass dry-run validation (leaf crates: full `cargo publish --dry-run`; non-leaf: `cargo package --list` since workspace deps not yet on crates.io)
- [x] **Check package sizes** — `cargo package -p sonos-sdk --list` for each public crate, ensure no unexpected files (plans/, brainstorms/)
