...
use sonos_discovery::{get, get_iter, get_iter_with_timeout, get_with_timeout, DeviceEvent};
    ...
    for event in get_iter_with_timeout(timeout) {
        match event {
            DeviceEvent::Found(device) => {
                ...
                assert!(
                    device.id.starts_with("uuid:"),
                    "Device ID should start with 'uuid:'"
                );

                // Verify IP address format (basic check)
                assert!(
                    device.ip_address.contains('.'),
...
fn test_early_iterator_termination_with_discovery() {
    ...
    println!("Early termination test: stopped after {count} device(s)");
}
...
fn test_get_convenience_function() {
    ...
    println!(
        "Convenience function test: found {} device(s)",
        devices.len()
    );
}
...
fn test_get_iter_default_timeout() {
    ...
    println!("Default iterator test: found {count} device(s)");
}
