...
impl StateStore {
    ...
    fn add_speaker(&mut self, speaker: SpeakerInfo) {
    ...
    fn get<P: Property>(&self, speaker_id: &SpeakerId) -> Option<P> {
        self.speaker_props.get(speaker_id)?.get::<P>()
    }

    fn set<P: Property>(&mut self, speaker_id: &SpeakerId, value: P) -> bool {
